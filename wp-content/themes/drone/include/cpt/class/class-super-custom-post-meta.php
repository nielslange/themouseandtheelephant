...
/**
	...
	public function add_media_field( $field, $post_meta, $html_attributes ) {
		...
		echo SCPT_Markup::tag( 'p', '',
			...
			), $html_attributes ), sprintf( __( 'Set %s', 'bijou' ), $field['label'] ) )
		);
	...
	public function add_multiple_media_field( $field, $post_meta, $html_attributes ) {
		...
		while($i < count($ids)){	
			if ( $ids[$i] ) {
				...
				if ( strpos( $attachment->post_mime_type, 'image/' ) === 0 ) {
					...
					$preview .= sprintf( '<br /><a href="#" class="scpt-remove-multiple_media-thumbnail">%s</a>', __( 'Remove', 'bijou' ) );
					$preview .= sprintf( '</div>');
				} 
			} else {
				$preview .= '';
		...
		$multiple_image_button = sprintf( '<a href="#" class="scpt-add-multiple_media">%s</a><a href="#" class="scpt-clear-multiple_media">%s</a>', __( 'Add Image', 'bijou' ), __( 'Clear Images', 'bijou' ) );
		
		echo SCPT_Markup::tag( 'p', '', $multiple_image_button );
		echo SCPT_Markup::tag( 'input', array(
			'type'  => 'hidden',
			'value' => $value,
			'name'  => $field['meta_key'],
